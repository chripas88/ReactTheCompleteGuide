...
const Ingredients = () => {
  ...
  const {isLoading, error, data, sendRequest, reqExtra, reqIdentifer, clear} = useHttp();
    ...
    sendRequest('https://react-hooks-update-e2a2d.firebaseio.com/ingredients.json', 'POST', JSON.stringify(ingredient), ingredient, 'ADD_INGREDIENT');
  }, [sendRequest]);
	
	const removeIngredientHandler = useCallback(ingredientId => {
  	sendRequest(`https://react-hooks-update-e2a2d.firebaseio.com/ingredients/${ingredientId}.json`, 'DELETE', null, ingredientId, 'REMOVE_INGREDIENT');
  }, [sendRequest]);
